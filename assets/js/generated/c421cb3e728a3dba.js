  ...
  try {
    if (authMode === "signin") {
      ...
      dsTrack('sign_up', {
        ...
        // Level 4a reward attribution (spec Phase F): which bucket this
        // visitor was in for every active client-side experiment, so GA4
        // can correlate exposure -> conversion per arm.
        ...(window.DsAbExperiments ? window.DsAbExperiments.getActiveBuckets(window.localStorage) : {}),
      });
    }
    setAuthMsg("");
    closeAuthModal();
