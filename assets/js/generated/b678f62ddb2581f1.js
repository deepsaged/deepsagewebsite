    ...
    // Server-verified IP lookup via the oauth.deepsage.com Worker, which
    // reads CF-Connecting-IP rather than trusting a client-reported value
    // (the previous third-party lookup this replaced was spoofable). Only
    // called at the moment of signup, never on every pageview, to keep the
    // third-party call and its privacy exposure minimal. Degrades to null
    // on any failure (offline, service down, blocked by an ad-blocker)
    // rather than breaking signup.
    async function fetchClientIp() {
      try {
        const res = await fetch('https://oauth.deepsage.com/api/client-ip');
